...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route("admin.helpdesks.update", [$helpdesk->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group {{ $errors->has('priority_id') ? 'has-error' : '' }}">
                ...
                <select name="priority_id" id="priority_id" class="form-control select2">
                    ...
                    <option value="{{ $id }}" {{ (isset($helpdesk) && $helpdesk->priorities ? $helpdesk->priorities->name : old('priority_id')) == $id ? 'selected' : '' }}>{{ $priority }}</option>
                    @endforeach
                </select>
