                ...
                <tbody>
                    @foreach($helpdesks as $key => $helpdesk)
                        <tr data-entry-id="{{ $helpdesk->id }}">
                            ...
                            <td>
                                <span class="countdown" data-entry-id="{{ $helpdesk->id }}" data-created-at="{{ $helpdesk->created_at }}" data-escalation-time="{{ $helpdesk->priority->escalation_time }}"></span>
                            </td>
                            ...
                            <td>{{ \Carbon\Carbon::parse($helpdesk->created_at)->timezone('Asia/Jakarta')->locale('id')->isoFormat('LLLL') ?? '' }}</td>
                            <td>
                                @can('helpdesk_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.helpdesks.show', $helpdesk->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                @can('helpdesk_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.helpdesks.edit', $helpdesk->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('helpdesk_delete')
                                    <form action="{{ route('admin.helpdesks.destroy', $helpdesk->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
