                    ...
                    <div class="card card-xxl-stretch">
                        ...
                        <div class="card-header align-items-center border-0 mt-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="ticket_number" class="form-control mt-3">
                                        ...
                                        @foreach ($allTickets as $ticket)
                                            <option value="{{ $ticket->id }}" {{ $selectedTicketId == $ticket->id ? 'selected' : '' }}>
                                                {{ $ticket->no_ticket }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <button type="submit" class="btn btn-primary mt-3">Filter</button>
                                    <a href="{{ route('customer.dashboard.index') }}" class="btn btn-secondary mt-3 ms-2">Refresh</a>
                                </div>
                            </form>
                            ...
                            <div class="col-xl-12 mt-6" style="max-height: 250px; overflow-y: auto;">
                                            ...
                                            <li class="timeline-item {{ $loop->first ? 'current-status' : '' }}">
                                                ...
                                                <div class="timeline-content">
                                                    ...
                                                    <p class="timeline-text">
                                                        ...
                                                        <strong>Ditugaskan Ke :</strong> {{ $log->assignTo->name ?? 'N/A' }}<br>
                                                        <strong>Jatuh Tempo :</strong> {{ \Carbon\Carbon::parse($log->h_due_date)->translatedFormat('d F Y') ?? 'N/A' }}<br>
                                                        ...
                                                        @if ($log->h_attachments)
                                                            @foreach (json_decode($log->h_attachments) as $attachment)
                                                                ...
                                                                @endphp
                                                                <a href="#" class="attachment-link" data-bs-toggle="modal" data-bs-target="#imageModal" data-src="{{ asset($attachment) }}">{{ $shortenedFilename }}</a><br>
                                                            @endforeach
                                                        @else
                                                            N/A
                                                        @endif
                                                        <br>
                                                        <strong>Status Diubah Oleh :</strong> {{ $log->statusChangedByUser->name ?? 'N/A' }}
                                                    </p>
                                                </div>
                                            </li>
                                        @endforeach
                                ...
                                @else
                                    <div class="text-center mt-5">
                                        <img src="{{ asset('template/dist/assets/media/illustrations/presentation.png') }}" alt="No History" class="img-fluid" style="width: 280px" height="150px" />
