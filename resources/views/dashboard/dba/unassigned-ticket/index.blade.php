                                    ...
                                    @foreach ($tickets as $ticket)
                                        <tr>
                                            ...
                                            <td class="d-flex justify-content-center align-items-center">
                                                @can('Show Ticket')
                                                    <a class="menu-link"
                                                        href="{{ route('unassignedDba.show', $ticket->id) }}" type="button">
                                                ...
                                                @endcan
                                                @can('Edit Ticket')
                                                    <a href="{{ route('unassignedDba.edit', $ticket->id) }}" title="Edit"
        ...
        <div class="modal fade" tabindex="-1" id="kt_modal_request_{{ $ticket->id }}">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <form action="{{ route('unassignedDba.verifyTicket', $ticket->id) }}"
                            method="POST" class="d-inline">
                            @csrf
                            @method('PUT')
                            <button class="btn btn-primary" type="submit">Verifikasi</button>
                        </form>
                        <form action="{{ route('unassignedDba.rejectTicket', $ticket->id) }}" method="POST">
                            @csrf
                            @method('PUT')
