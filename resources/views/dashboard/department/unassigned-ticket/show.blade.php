            ...
            <div class="card-body">
                ...
                <div class="tab-content">
                                            ...
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div class="d-flex flex-column">
                                                    <h1 class="text-primary fw-bold mb-2">
                                                        {{ $ticket->category->category_name ?? 'Tiket' }} ({{ $ticket->no_ticket }})
                                                    </h1>

                                                    <div class="text-muted mb-1">
                                                        <i class="fas fa-user me-2"></i>
                                                        <span class="fw-bolder text-dark">{{ $ticket->customers->name }} (Pemilik)</span>
                                                    </div>
                                                ...
                                                <div id="kt_accordion_comments_item_1" class="collapse fs-6 ps-10" data-bs-parent="#kt_accordion_comments">
                                                    <div class="card shadow-sm">
                                                                    ...
                                                                    <div class="d-flex justify-content-{{ $comment->user_id == auth()->user()->id ? 'end' : 'start' }} mb-10" id="comment-{{ $comment->id }}">
                                                                        <div class="d-flex flex-column align-items-{{ $comment->user_id == auth()->user()->id ? 'end' : 'start' }}">
                                                                            ...
                                                                            <div class="p-5 rounded bg-light-{{ $comment->user_id == auth()->user()->id ? 'primary' : 'info' }} text-dark fw-bold mw-lg-600px text-{{ $comment->user_id == auth()->user()->id ? 'end' : 'start' }}" data-kt-element="message-text">
                                                                                ...
                                                                                <form action="{{ route('unassignedTickets.update', $comment->id) }}" method="POST" class="comment-form" data-comment-id="{{ $comment->id }}">
                                                                                    @method('PUT')
                                                                                    @csrf
                                                        ...
                                                        <div class="card-footer pt-4" id="kt_chat_messenger_footer">
                                                            <form class="row g-3 mt-2 needs-validation" method="POST" action="{{ route('unassignedTickets.store') }}" enctype="multipart/form-data" novalidate>
                                                                @csrf
                        ...
                        <div class="col-xl-12">
                            <div class="card shadow-sm">
                                <div class="card-header bg-primary text-white">
                                    <h3 class="mt-8 text-white">Lampiran Tiket</h3>
                                </div>
                                <div class="card-body">
                                            ...
                                            @foreach ($attachments as $index => $attachment)
                                                <div class="p-2">
                                                    <div class="card shadow-sm" style="width: 160px;">
                                                        <img src="{{ asset($attachment) }}" alt="{{ basename($attachment) }}" class="rounded-top" style="width: 100%; height: 120px; object-fit: cover; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#kt_modal_{{ $ticket->id }}_{{ $index }}" />
                                                        <div class="card-body text-center p-2">
                                                            <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_{{ $ticket->id }}_{{ $index }}">
                                                                Lihat Lampiran
                                                            </button>
                                                        </div>
                                                    </div>
                            ...
                            @if (count($attachments) > 0 && !empty($attachments[0]))
                                @foreach ($attachments as $index => $attachment)
                                    <div class="modal fade" id="kt_modal_{{ $ticket->id }}_{{ $index }}" tabindex="-1" aria-hidden="true">
                                        <div class="modal-dialog modal-dialog-centered modal-lg">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body text-center">
                                                    <img src="{{ asset($attachment) }}" alt="{{ basename($attachment) }}" class="img-fluid rounded shadow-sm" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="tab-pane fade" id="riwayat" role="tabpanel">
                        <div class="col-xl-12">
                            <div class="card card-xl-stretch mb-5 mb-xl-8 scrollable-card activity-log" style="max-height: 756px; overflow-y: auto;">
                                <div class="card-body p-12">
                                    ...
                                    <ul class="timeline">
                                        @foreach ($logs as $log)
                                            <li class="timeline-item {{ $loop->first ? 'current-status' : '' }}">
                                                ...
                                                <div class="timeline-content">
                                                    ...
                                                    <p class="timeline-text">
                                                        ...
                                                        <strong>Nama Pemilik :</strong> {{ $log->customers->name ?? 'N/A' }}<br>
                                                        <strong>Prioritas :</strong> {{ $log->priority->priority_name ?? 'N/A' }}<br>
                                                        <strong>Status :</strong> {{ $log->status->status_name ?? 'N/A' }}<br>

                                                        <strong>Lampiran :</strong>
                                                        @if ($log->h_attachments)
                                                            <div class="row row-cols-auto g-2 mt-3">
                                                                @foreach (json_decode($log->h_attachments) as $index => $attachment)
                                                                    <div class="col">
                                                                        <img src="{{ asset($attachment) }}" alt="{{ basename($attachment) }}" class="rounded shadow-sm img-thumbnail" style="width: 150px; height: 150px; object-fit: cover; cursor: pointer;" data-bs-toggle="modal" data-bs-target="#riwayat_modal_{{ $log->id }}_{{ $index }}" />
                                                                    </div>
                                                                @endforeach
                                                            </div>
                                                        @else
                                                    ...
                                                    <br>
                                                    <strong>Status Diubah Oleh :</strong> {{ $log->statusChangedByUser->name ?? 'N/A' }}<br>
                                                    </p>
                                                </div>
                                            </li>
                                            ...
                                            @if ($log->h_attachments)
                                                @foreach (json_decode($log->h_attachments) as $index => $attachment)
                                                    <div class="modal fade" id="riwayat_modal_{{ $log->id }}_{{ $index }}" tabindex="-1" aria-hidden="true">
                                                        <div class="modal-dialog modal-dialog-centered modal-lg">
                                                            <div class="modal-content">
                                                                 <div class="modal-header">
                                                                ...
                                                                <div class="modal-body text-center">
                                                                    <img src="{{ asset($attachment) }}" alt="{{ basename($attachment) }}" class="img-fluid rounded" />
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            editButtons.forEach(button => {
                button.addEventListener('click', function () {
                    ...
                    const displayParagraph = document.getElementById(`message-display-${commentId}`);
                    ...
                    document.querySelectorAll('.update-button').forEach(el => el.style.display = 'none');
                    if (currentEditor) {
                    ...
                    document.querySelectorAll('p[id^="message-display-"]').forEach(el => el.style.display = 'block');
                    if (displayParagraph) {
